...
def savitzky_golay_smooth(data, window_size=5, poly_order=2):
    ...
    return np.array(
        [
            savgol_filter(data[:, :, i], window_size, poly_order)
...
class Simulation:
    ...
    def simulation_lorenz(self, lorenz_params):
        for sample in range(self.num_samples):
            self.simulation[sample] = self.lorenz_simulation(
                self.initial_points[sample], **lorenz_params
            )
    ...
    def simulation_line(self):
        ...
        for sample in range(self.num_samples):
            ...
            self.simulation[sample] = generate_3d_line(
                self.initial_points[sample], new_direction, num_steps=self.num_steps
            )
