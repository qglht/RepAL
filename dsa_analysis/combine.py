...
def attach_2_motifs(motifA, motifB):
    ...
    new_motif[motifA.shape[0]] = new_motif[motifA.shape[0]-1] + 0.0001*(motifA[-1] - motifA[-2])
    for i in range(1,motifB.shape[0]):
        new_motif[motifA.shape[0]+i] = new_motif[motifA.shape[0]+i-1] + motifB[i] - motifB[i-1]
    return new_motif

def combine_2_motifs(motifA, motifB, mixing_level=0.5):
    """Combine two motifs by concatenating them along the time axis.
    ...
    for i in range(motifB.shape[0]):
        new_motif[i] = (1-mixing_level)*motifB[i] + mixing_level*(motifA[i]- motifA[0])
    return new_motif
...
def combine_motifs(motifs:List[np.ndarray], method='attach',mixing_level=0):
    ...
    if method=='attach':
        ...
        for i in range(2,len(motifs)):
            new_motif = combine_2_motifs(new_motif, motifs[i], mixing_level=mixing_level)
    return new_motif


def combine_simulations(simulations:List[np.ndarray], method='attach', mixing_level=0):
    combined_simulation = np.empty((simulations[0].shape[0], simulations[0].shape[1], simulations[0].shape[2]))
    for i in range(simulations[0].shape[0]):
        combine_simulation_unormalized = combine_motifs([simulation[i] for simulation in simulations], method=method, mixing_level=mixing_level)
        combined_simulation[i]= downsample(combine_simulation_unormalized , len(simulations)) if method=='attach' else combine_simulation_unormalized
        combined_simulation[i] = normalize_within_unit_volume(combined_simulation[i])
