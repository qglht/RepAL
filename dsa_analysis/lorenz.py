...
def perturb_curve_diffusion(curve, perturbation_scale: float = 0.02):
    ...
    perturbed_curve = np.sqrt(1-perturbation_scale**2)*perturbed_curve + perturbation_scale*perturbation
    return perturbed_curve
...
def lorenz_simulation(dt=0.01, num_steps=10000, initial_point=(0., 1., 1.05), perturbation_scale: float= 0.02, sigma:float= 2.0, order: int=0, **lorenz_params):
...
def simulation(dt, lorenz_parameters, num_samples, num_steps, perturbation: bool= False, perturbation_scale: float= 0.1, smoothing: bool= False, sigma:float= 2.0):
    ...
    for i, (_, params) in enumerate(lorenz_parameters.items()):
      ...
      for param in params:
         ...
         for sample in range(num_samples):
            simulations[condition, sub_condition, sample] = lorenz_simulation(dt, num_steps, initial_point=initial_points[sample], perturbation=perturbation, perturbation_scale=perturbation_scale, smoothing=smoothing, sigma=sigma, **param)
    return simulations
...
def simulation_line(num_steps, num_samples):
    ...
    for sample in range(num_samples):
        ...
        simulations[sample] = generate_3d_line(initial_points[sample], new_direction, num_steps=num_steps)
    return simulations

def simulation_lorenz(dt, lorenz_parameters, num_samples, num_steps, perturbation_scale: float= 0.02, order:int=0, sigma:float= 2.0):
    """Run simulations for different Lorenz parameter conditions."""
    simulations = np.empty((num_samples, num_steps, 3))
    initial_points = generate_initial_points(num_samples)
    param = lorenz_parameters
    for sample in range(num_samples):
        simulations[sample] = lorenz_simulation(dt, num_steps, initial_point=initial_points[sample], perturbation_scale=perturbation_scale, sigma=sigma, order=order, **param)
    return simulations