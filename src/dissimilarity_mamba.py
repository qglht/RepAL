...
(poetry run python -m src.dissimilarity_pairwise_mamba --taskset {taskset} --group1 {group1} --group2 {group2}) &

# PID of the application
APP_PID=$!

# Log file path
LOG_FILE=gpu_usage/{taskset}_{group1}_{group2}_gpu_usage.log
    ...
    for i in range(len(groups)):
        ...
        for j in range(i, len(groups)):
            ...
            if not os.path.exists(
                f"data/dissimilarities/mamba/{args.taskset}/{group_i}_{group_j}.csv"
            ):
